...
const AccountPage = () => {
    ...
    const [newAccountName, setNewAccountName] = useState(account.fullName)
    ...
    const handleUpdateAccount = async () => {
        ...
        try {
            ...
            if (newAccountName != account.fullName)
                updatedData.fullName = newAccountName;
            ...
            console.error(error.response ? error.response.data : error.message);  // In chi tiết lỗi từ server
    ...
    const getTotalAmount = async (ticketId) => {
        setLoading(true);
        try {
            const response = await axios.get(`http://localhost:8080/api/ticket/totalAmountByUserId/${ticketId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                }
            });
    ...
    useEffect(() => {
        if (account && account.user_id) {
            getTotalAmount(account.user_id);
        }
    }, [account]);
        ...
        <div className="bg-white-10">
                    ...
                    <div className="bg-white px-6 md:p-4 xl:px-6 xl:py-4  xl:shadow-2xl rounded mb-8">
                        ...
                        <div className="info__money__rating py-6 xl:border-b border-[#ECECEC]">
                            <div className="flex justify-between items-center ">
                                <p className="md:text-base xl:text-lg font-bold not-italic relative">Tổng chi tiêu {currentYear}</p>
                                <span className="md:text-base xl:text-lg font-bold not-italic text-[#F58020]">{Number(totalAmount).toLocaleString()}&nbsp;₫</span>
                            </div>
                        </div>
                        <InfoSupport />
                    </div>
            ...
            <Modal open={isNameModalOpen} onClose={closeModal} center
                ...
                <div className="bg-white rounded-lg p-6">
                    <h2 className="text-lg font-semibold mb-4">Thay đổi họ tên</h2>
                    <TextField
                        label="Full Name"
                        fullWidth
                        variant="outlined"
            ...
            <Modal open={isBirthdayModalOpen} onClose={closeModal} center
                ...
                <div className="bg-white rounded-lg p-6">
                    <h2 className="text-lg font-semibold mb-4">Thay đổi ngày sinh</h2>
                    <TextField
                        type="date"
                        label="Birthday"
                        fullWidth
        ...
        <div className="pt-4 pb-6 xl:border-b border-[#ECECEC] flex justify-center items-center ">
            <div className="flex items-center">
                <div className="w-[72px] h-[72px] leading-[62px] text-center rounded-full bg-[#D0D0D0] border-4 border-solid border-[#E9E9E2]">
                    <img alt="Camera" loading="lazy" decoding="async" data-nimg="1" className="inline-block rounded-full w-full h-full object-cover duration-500 ease-in-out group-hover:opacity-100 scale-100 blur-0 grayscale-0)" src={account.image} style={{ color: "transparent" }} />
                </div>
                <div className="flex flex-col flex-auto">
                    <div className="flex items-start gap-[6px]">
                        <p className="text-[18px] font-bold not-italic leading-[1.25rem] ml-3">{account.fullName}
        ...
        <div className="info__support border-0">
            <ul>
                ...
                <li className="py-4 border-y border-[#ECECEC]"><a className="cursor-pointer flex justify-between items-center" href="mailto:hotro@galaxystudio.vn">
                    <h3 className="text-sm font-bold not-italic text-[#034EA2]"><strong className="text-[#333333]">Email:</strong> hotro@mhstudio.vn</h3>
                    <span>
                        <svg width="7" height="12" viewBox="0 0 7 12" fill="none"><path d="M4.58579 6L0.292893 1.70711C-0.0976311 1.31658 -0.0976311 0.683417 0.292893 0.292893C0.683418 -0.0976305 1.31658 -0.0976305 1.70711 0.292893L6.70711 5.29289C7.09763 5.68342 7.09763 6.31658 6.70711 6.70711L1.70711 11.7071C1.31658 12.0976 0.683418 12.0976 0.292893 11.7071C-0.0976311 11.3166 -0.0976311 10.6834 0.292893 10.2929L4.58579 6Z" fill="#777777"></path></svg>
                    </span>
                </a>
