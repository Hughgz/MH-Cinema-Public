...
const truncateText = (text, maxWords) => {
    // Ensure text is a valid string before calling .split()
                        ...
                        {filteredMovies.map(movie => (
                            <ListItem key={movie.id} alignItems="flex-start">
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column' }}>
                                    <ListItemText
                                        primary={<a href={`/chon-phim/${movie.movieUrl}`} style={{ textDecoration: 'none', color: '#1976d2', fontSize: '1.5rem' }}>{movie.name}</a>}
                                    />
                                    <Typography variant="body2" color="textSecondary" style={{ fontSize: '0.875rem', marginTop: '0.5rem' }}>
                                        {truncateText(movie.shortDescription, 30)}
                                    </Typography>
                                </div>
                            </ListItem>
                        ))}
