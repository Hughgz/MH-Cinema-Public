...
const MovieNBExtended = () => {
    ...
    const filteredMovies = movies
        .filter(movie => movie.showing === 1)
        .sort((a, b) => new Date(b.releaseDate) - new Date(a.releaseDate)).slice(0, 4);
                ...
                <div className="movie__show">
                    ...
                    <ul className="flex flex-row gap-7 justify-between">
                        {filteredMovies.map((movie) => (
                            <MovieCardItem key={movie.id} title={movie.name} imageUrl={movie.smallImgMovie} href={movie.movieUrl} id={movie.id} trailer={movie.trailerURL} />
                        ))}
                    </ul>
                </div>
            </div>
