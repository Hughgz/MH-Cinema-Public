...
const MovieOverviewComponent = ({movie}) => {
    return (
        <div className="movie__info relative md:grid hidden grid-cols-3 md:gap-5 gap-3 lg:items-end ">
            ...
            <img alt="Linh Miêu: Quỷ Nhập Tràng" loading="lazy" width="220" height="280" decoding="async" data-nimg="1" className="border-2 rounded border-white lg:w-[320px] lg:h-[400px] w-full h-full object-fill object-cover duration-500 ease-in-out group-hover:opacity-100&quot;
      scale-100 blur-0 grayscale-0)" src={movie.smallImgMovie} style={{color: "transparent;"}}></img>
            </div>
            <div className="col-span-2 lg:-translate-y-20 flex flex-col justify-end md:-translate-y-16 -translate-y-0">
                ...
                <div className="flex items-center">
                    ...
                    <div className="text-sm ml-4 flex items-center font-semibold not-italic">
                        ...
                        <span>{`${movie.releaseDate[0]} - ${String(movie.releaseDate[1]).padStart(2, '0')} - ${String(movie.releaseDate[2]).padStart(2, '0')}`}</span>
                    </div>
                </div>
                <div className="mt-2">
                    <a className="text-[20px] hover:text-primary transition duration-500 ease-in-out cursor-pointer">
                        ...
                        <span className="inline-block mr-1">{movie.rated}</span>
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <LabelLineComponent labeLineTitle="Thể loại" labelList={[movie.categories]} />
                    <LabelLineComponent labeLineTitle="Đạo diễn" labelList={[movie.director]} />
                    <LabelLineComponent labeLineTitle="Diễn viên" labelList={[movie.actors]} />
                </div>
            </div>
        </div>
